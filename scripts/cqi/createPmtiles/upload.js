...
console.log(
  'START',
  __filename,
  'Uploading all files from ./files_pmtiles to S3 bucket atlas-tiles',
)

const pmtilesFiles = fs
  .readdirSync(path.resolve(__dirname, './files_pmtiles'))
  .filter((file) => path.extname(file) === '.pmtiles')

for (const file of pmtilesFiles) {
  const Key = file
  const filePath = path.resolve(__dirname, './files_pmtiles', file)
