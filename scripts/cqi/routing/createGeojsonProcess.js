...
console.log('START WITH', inputFile)
if (!inputFile) {
  console.error(
...
const routingSzenarios = new Map([
  [
    'base', // original file but transformed to the routing format with just one geometry per way
    {
      lts1factor: 1,
      lts2factor: 1,
      lts3factor: 1,
...
for (const [routingSzenarioName, routingSzenarioWeight] of routingSzenarios) {
  const inputFilename = path.basename(inputFile).replace('.geojson', '').replace('_epsg4326', '')
  ...
  // DONE
  console.log('DONE', path.parse(outputFile).name)
