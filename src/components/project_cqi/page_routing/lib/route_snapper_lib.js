...
export class RouteSnapper {
  ...
  editExisting(feature) {
    if (!this.loaded) {
      ...
      console.error('editExisting called before the map idle event received. Not starting tool.')
      return
    }
  ...
  #finishSnapping() {
    ...
    if (rawJSON) {
      this.controlDiv.dispatchEvent(new CustomEvent('new-route', { detail: JSON.parse(rawJSON) }))
    } else {
      this.controlDiv.dispatchEvent(new CustomEvent('no-new-route'))
    }
    this.stop()
