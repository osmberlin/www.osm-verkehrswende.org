// Copied from https://github.com/dabreegster/ltn/blob/main/web/src/common/snapper/route_tool.ts
...
export class RouteTool {
  ...
  onMouseMove = (e: MapMouseEvent) => {
    ...
    const nearbyPoint: [number, number] = [e.point.x - snapDistancePixels, e.point.y]
    const circleRadiusMeters = this.map
      .unproject(e.point)
      .distanceTo(this.map.unproject(nearbyPoint))
    if (this.inner.onMouseMove(e.lngLat.lng, e.lngLat.lat, circleRadiusMeters)) {
      this.redraw()
  ...
  editExistingRoute(feature: Feature<LineString>) {
    ...
    if (!feature.properties.waypoints) {
      ...
      feature.properties.waypoints = [
        ...
        {
          lon: feature.geometry.coordinates[feature.geometry.coordinates.length - 1][0],
  ...
  editExistingArea(feature: Feature<Polygon>) {
    ...
    if (!feature.properties.waypoints) {
      window.alert('Bug: editExistingArea called for a polygon not produced by the route-snapper')
    }
  ...
  addEventListenerSuccess(callback: (f: Feature<LineString | Polygon>) => void) {
    this.eventListenersSuccess.push(callback)
  }
  addEventListenerUpdated(callback: (f: Feature<LineString | Polygon>) => void) {
  ...
  setRouteConfig(config: { avoid_doubling_back: boolean; extend_route: boolean }) {
